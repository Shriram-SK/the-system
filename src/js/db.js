...
async function insertQuests(userId, quests) {
  ...
  const { data, error } = await supabase.from('daily_quests').insert(rows).select();
  if (error) throw error;
  return data;
}
...
async function insertHabit(userId, name, xpPerDay, stat) {
  const { data, error } = await supabase
    .from('habits')
    .insert({ player_id: userId, name, xp_per_day: xpPerDay, stat_bonus: stat })
    .select()
    .single();
  if (error) throw error;
  return data;
...
async function insertCustomReward(userId, name, icon, cost) {
  const { data, error } = await supabase
    .from('custom_rewards')
    .insert({ player_id: userId, name, icon, cost })
    .select()
    .single();
  if (error) throw error;
  return data;
...
async function deleteCustomReward(rewardId) {
  const { error } = await supabase.from('custom_rewards').delete().eq('id', rewardId);
  if (error) throw error;
}

// ── PURCHASES ─────────────────────────────────────
async function logPurchase(userId, rewardName, cost) {
  await supabase.from('purchases').insert({ player_id: userId, reward_name: rewardName, cost });
}
